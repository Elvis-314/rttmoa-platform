...
// setLocalStorage("history_paths", {})
export const setLocalStorage = (key: string, value: any) => {
	window.localStorage.setItem(key, JSON.stringify(value));
};
// removeLocalStorage("history_paths")
...
export const camelizeCamelCase = (str: string) =>
	str.replace(/(?:^\w|[A-Z]|\b\w)/g, (letter: string, index: number) => (index === 0 ? letter.toLowerCase() : letter.toUpperCase())).replace(/\s+/g, '');
